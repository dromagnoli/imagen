...
public class ComparisonTest extends ComparisonTestBase {
    ...
    public void testOperation(int dataType, TestSelection testType) {
        ...
        for (int statistic = 0; statistic < 3; statistic++) {
            ...
            if (OLD_DESCRIPTOR) {
                /*  if (statistic == 0) {
                    image = org.eclipse.imagen.operator.MeanDescriptor.create(testImage, roi,
                            xPeriod, yPeriod, null);
                } else if (statistic == 1) {
                    image = org.eclipse.imagen.operator.ExtremaDescriptor.create(testImage, roi,
                            xPeriod, yPeriod, false, 1, null);
                } else if (statistic == 2) {
                    image = org.eclipse.imagen.operator.HistogramDescriptor.create(testImage,
                            roi, xPeriod, yPeriod, numBinsTest, minBoundsTest, maxBoundsTest, null);
                }*/
            } else {
                image = StatisticsDescriptor.create(
                        testImage,
                        xPeriod,
                        yPeriod,
                        roi,
