...
public class ComparisonTest extends ComparisonTestBase {
    ...
    protected boolean supportDataType(int dataType) {
        if (dataType == DataBuffer.TYPE_SHORT) return false;
        else return super.supportDataType(dataType);
    }

    @Test
    public void testDataTypesWithRoi() {
        if (!OLD_DESCRIPTOR) testAllTypes(TestSelection.ROI_ONLY_DATA);
    }

    @Test
    public void testDataTypesWithNoData() {
        if (!OLD_DESCRIPTOR) testAllTypes(TestSelection.NO_ROI_NO_DATA);
    }

    @Test
    public void testDataTypesWithBoth() {
        if (!OLD_DESCRIPTOR) testAllTypes(TestSelection.ROI_NO_DATA);
    }
    ...
    public void testOperation(int dataType, TestSelection testType) {
        ...
        if (OLD_DESCRIPTOR) {
            /*image = org.eclipse.imagen.media.crop.CropDescriptor.create(testImage, cropX, cropY,
            cropWidth, cropHeight, null);*/
        } else {
            image = CropDescriptor.create(testImage, cropX, cropY, cropWidth, cropHeight, roi, range, null, null);
        }
